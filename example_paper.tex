...
\twocolumn[
\icmltitle{A Semiparametric Framework for Modeling\\Patient Disease Dynamics}
...
\begin{abstract}
The availability of temporal clinical data presents new opportunities for modeling chronic and acute disease conditions.  However, electronic health and medical data is the US is often arbitrarily sampled, variable in length, and incomplete.  In this paper, we describe a semi-parametric clustering framework that can be used as a preprocessing or exploratory analysis step that is designed to avoid the limitations of methods that discretize time series sequences and that require the number of clusters to be indicated a priori.  This methods shows a 20\% relative imporvement on an alternative method published in the biomedical informatics literature.
\end{abstract}
